...
class Harness
  SYSTEM_PROMPT = <<~'TEXT'
    ...
    2. Use the "file_read" tool to read the current contents of files you need to modify. It returns the file's SHA-256 digest along with its contents.
    3. Make the requested changes.
    4. Use the "file_write" tool to write the complete modified file content back. You must pass the SHA-256 digest you obtained from file_read (or file_sha) as the "sha" argument; it is verified to match the file on disk before writing. If the file has changed since you read it, the write is rejected — re-read the file and retry.
    5. Only modify files that are in the provided list of available files.
    6. Each file write must contain the COMPLETE file content (not a diff or snippet).
    7. Preserve original formatting, indentation, and style unless the instruction says otherwise.
