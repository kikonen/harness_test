...
# (use file_delete to remove their contents first) — there is deliberately
...
class DirDeleteTool < Tool
  ...
  def execute(args)
    ...
    unless path.start_with?(prefix)
      puts "  [dir_delete] ✗ #{shown} (outside working directory)"
      $stdout.flush
    ...
    if @file_list.sensitive?(path)
      puts "  [dir_delete] ✗ #{shown} (blocked: sensitive path)"
      $stdout.flush
      return "error: path '#{shown}' is blocked and can never be deleted"
    end

    unless File.directory?(path)
      puts "  [dir_delete] ✗ #{shown} (not a directory)"
      $stdout.flush
    ...
    unless entries.empty?
      puts "  [dir_delete] ✗ #{shown} (not empty: #{entries.size} entr#{entries.size == 1 ? 'y' : 'ies'})"
      $stdout.flush
    ...
    puts
    puts "  [dir_delete] ⚠  The model is requesting to DELETE an empty directory:"
    puts "                  #{shown}"
    print  "                  Delete? (y/n): "
    $stdout.flush
    ...
    if answer == 'y' || answer == 'yes'
      Dir.rmdir(path)
      puts "  [dir_delete] ✓ #{shown} (deleted)"
      $stdout.flush
      "ok: empty directory '#{shown}' has been deleted"
    else
      puts "  [dir_delete] ✗ #{shown} (denied by user)"
      $stdout.flush
