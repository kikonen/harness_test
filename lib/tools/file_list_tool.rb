...
# still use file_add (with user confirmation) to work with a file.
class FileListTool < Tool
  ...
  def initialize(file_list)
    ...
    super(
      ...
      description: 'Lists files matching a glob pattern under the harness working directory. ' \
                   ...
                   'Matched files are NOT added to the allowed file list — use file_add to add a file you want to work with.',
      parameters: {
        type: 'object',
        properties: {
  ...
  def execute(args)
    ...
    return 'error: usage: file_list(pattern)' if pattern.empty?
