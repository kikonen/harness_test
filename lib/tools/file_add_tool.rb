...
class FileAddTool < Tool
  ...
  def execute(args)
    ...
    puts
    puts "  [file.add] ⚠  The model is requesting to add a file to the allowed list:"
    ...
    if answer == 'y' || answer == 'yes'
      @file_list.add(path)
      puts "  [file.add] ✓ #{shown} (#{file_status} — added to allowed list)"
      $stdout.flush
      "ok: file '#{shown}' (#{file_status}) has been added to the allowed file list"
    else
      puts "  [file.add] ✗ #{shown} (#{file_status} — denied by user)"
      $stdout.flush
